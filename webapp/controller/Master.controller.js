/*global history */
sap.ui.define([
	"dlyRts/DeliveryRoutes/controller/BaseController",
	...
	"sap/ui/Device",
	"dlyRts/DeliveryRoutes/model/formatter",
	"sap/m/MessageBox"
	...
	return BaseController.extend("dlyRts.DeliveryRoutes.controller.Master", {
		...
		onInit: function () {
			...
			this.setModel(oSortModel, "sortData");
		...
		onSearch: function (oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("Ablad", FilterOperator.Contains, sQuery)];
			} else {
		...
		onSemanticSelectChange: function (oEvent) {
			var sSortField = oEvent.getSource().getSelectedItem().getKey();
			// 	aSorters = this._oGroupSortState.sort(sKey);
			// this._applyGroupSort(aSorters);
		...
		_onMasterMatched: function () {
			this._oListSelector.oWhenListLoadingIsDone.then(
				function (mParams) {
					...
					var sAblad = mParams.firstListitem.getBindingContext().getProperty("Ablad"),
						sWerks = mParams.firstListitem.getBindingContext().getProperty("Werks"),
						sKstrg = mParams.firstListitem.getBindingContext().getProperty("Kstrg");
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							Ablad: encodeURIComponent(sAblad),
							Werks: encodeURIComponent(sWerks),
							Kstrg: encodeURIComponent(sKstrg)
						}, true);
					}
				}.bind(this),
				function (mParams) {
		...
		_showDetail: function (oItem) {
			var sAblad = oItem.getBindingContext().getProperty("Ablad"),
				sWerks = oItem.getBindingContext().getProperty("Werks"),
				sKstrg = oItem.getBindingContext().getProperty("Kstrg");
